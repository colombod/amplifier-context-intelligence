...
def create_asgi_app(
    ...
    *_jwks_client* is an injectable JWKS client used **only** when
    ...
    if s.auth_mode == "entra":
        ...
        # identity source; checked against the CONFIG maps (not the merged
        # store's flat_dict, which now holds both kinds by design).
        ...
        # Pass entra_store.flat_dict as BOTH maps — SAME live reference, so
        # admin edits (PUT /admin/identities) resolve with no restart required.
        resolver: StaticKeyResolver | EntraResolver = EntraResolver(
            ...
            service_identity_map=entra_store.flat_dict,  # SAME live reference (#501)
