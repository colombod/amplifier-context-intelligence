...
class TestM2CapabilityDeps:
    ...
    async def test_cap_sc_contributor_write_capable(
        ...
        Uses service_asgi_with_map (mapped oid) since #501 requires a service
        identity map hit -- an unmapped oid now 403s before role gating even runs.
        """
        import context_intelligence_server.main as main_module  # noqa: PLC0415
    ...
    async def test_cap_sr_r_reader_read_capable(
        ...
        Uses service_asgi_with_map (mapped oid) -- see CAP-SC docstring (#501).
        """
        import context_intelligence_server.main as main_module  # noqa: PLC0415
    ...
    async def test_cap_sadm_a_admin_role_passes_require_admin(
        ...
        (mapped oid) -- see CAP-SC docstring (#501).
        """
        private_key, asgi = service_asgi_with_map
        token = _sign_jwt(private_key, _service_claims(roles=["IdentityAdmin"]))
    ...
    async def test_cap_cypher_read_reader_can_reach_cypher(
        ...
        service_asgi_with_map (mapped oid) -- see CAP-SC docstring (#501).
        """
...
class TestBootDisjointnessInvariant:
    ...
    def test_empty_service_identities_boots_without_error(
        ...
        """#501: service_identities={} boots cleanly (bootstrap state, not a startup error).

        Before #501 this was a hard ValidationError. An explicit empty map is
...
class TestP3VerticalSlice:
    ...
    async def test_p3_unmapped_service_contributor_is_rejected(
        ...
        The resolver now fails loud (issue #501, mirrors the user branch's
...
class TestM2MessageContent:
    ...
    async def test_require_write_403_names_write_role(
        ...
        (#501: an unmapped oid now 403s before require_write ever runs).
        """
        private_key, asgi = service_asgi_with_map
        token = _sign_jwt(private_key, _service_claims(roles=["Reader"]))
    ...
    async def test_require_read_403_names_reader_and_write_roles(
        ...
        Uses service_asgi_with_map (mapped oid) so the resolver itself succeeds
        (#501: an unmapped oid now 403s before require_read ever runs).
        """
        private_key, asgi = service_asgi_with_map
        token = _sign_jwt(private_key, _service_claims(roles=["IdentityAdmin"]))
