...
> **Superseded by issue #501.** The resolver's `created_by` **fallback chain
...
> now-removed fallback. Re-running this gate after #501 requires the test
...
- `appid` / `azp` are present and stable claims on the token — **superseded by
  issue #501**: neither is ever used for `created_by` any more. The resolver
...
4. *(pre-#501 assertion, now superseded)* `appid` or `azp` is present — this
   ...
   resolver; the real requirement post-#501 is that the token's `oid` is
...
- `POST /events` with the real token → asserts **HTTP 202**. **Post-#501,
  this now requires the caller's `oid` to already be mapped** in the shared
...
- Polls `POST /cypher` (best-effort, ~30s) for a node with
  ...
  *(pre-#501 this checked `created_by == appid`; that fallback no longer
...
> pre-#501 behavior — `azp` fed the old fallback chain. Post-#501 `created_by`
> comes solely from the shared identity store's mapped contributor id; `azp`
...
- Resolver under test: `context_intelligence_server/auth.py` —
  `EntraResolver.resolve()`, the `scp`/`idtyp` discriminator, and (post-#501)
