...
- **`EntraResolver` (auth_mode=entra) — M2 dual-path:** JWKS signing-key fetch → `jwt.decode`
  ...
  - **SERVICE path** (`idtyp == "app"` AND `scp` absent — M2 new): **role gate is the sole
    ...
    **shared identity store** (issue #501 — the same store backing `entra_identities`;
...
- **`entra` (M2 updated; reshaped by issue #501):** seeds **one shared `IdentityStore`**
  ...
  1. **First-boot seed (#501):** `build_identity_map()` (`entra_identities`, entries
...
> **Issue #501 update:** service identities are now managed through this **same**
